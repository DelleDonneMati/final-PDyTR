...
public class Ftp extends FtpGrpc.FtpImplBase {
    ...
    private static long totalReadTransferTime = 0;
    private static long totalWriteTransferTime = 0;
    ...
    public void read(ReadRequest request, StreamObserver<ReadResponse> responseObserver) {
        try {
            ...
            long transferTime = endTime - startTime; // Tiempo total de transferencia en milisegundos

            totalReadTransferTime += transferTime;

            System.out.println("Tiempo de lectura: " + transferTime + " ms");
            System.out.println("Tiempo total de lectura: " + totalReadTransferTime + " ms");
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void write(WriteRequest request, StreamObserver<WriteResponse> responseObserver) {
        try {
            ...
            totalWriteTransferTime += transferTime;

            System.out.println("Tiempo de escritura: " + transferTime + " ms");
            System.out.println("Tiempo total de escritura: " + totalWriteTransferTime + " ms");
        } catch (Exception e) {
            e.printStackTrace();
        }
        responseObserver.onCompleted();
